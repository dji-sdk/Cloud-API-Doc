...
* Function set > Dock Function Set: added command flight tutorial.
* Server API > MQTT:
  * Wayline management: added `pause wayline task` and `recover wayline task`.
  * Remote Log: added support for minio. 
  * Topic Definition: added DRC Topic and struct example.
  * Device Property: added device properties of command flight on aircraft property and dock property.
  * Added command flight function:
    * Flight control: flight direction control, gimbal rotation control, one-key taking off and flyto.
    * Payload control: photo taking, video recording, stop recording, double tab to become AIM and so on.
    * Events pushing: obstacle avoidance, HSI information, delay information of live streaming link.
* Error Code: added error code of command flight.
