...
* Firmware properties report
  ...
  We have unified the properties in the object model, please refer to the [object model properties]() chapter under the cloud API for details.

* Issue firmware upgrade task
  
  The firmware upgrade API delivered by the cloud needs to complete the filling of the device serial number, firmware upgrade package information, and firmware upgrade type. For the complete fields, meanings, and usage methods, please refer to [Firmware Upgrade]().

* Firmware upgrade progress
  
  The device reports the firmware upgrade progress to the cloud, and through the acquisition of fields, key information such as upgrade progress, upgrade progress percentage, and current upgrade steps can be displayed on the cloud visualization interface. Use the `ota_progress` field of the `firmware upgrade progress` API to determine whether the upgrade is in progress. For details, please refer to [Firmware upgrade progress]().
