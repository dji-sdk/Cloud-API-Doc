...
Waypoints library provides an ability that the flight path can be downloaded or uploaded in the same workspace. After logging in to the server through Pilot2, the user can obtain the waypoints list and display it in the waypoints library. In Pilot2, the waypoints existing in the cloud can be previewed directly, but when the user wants to edit the waypoints, it needs to be downloaded in DJI Pilot 2 locally and it will not affect the original waypoints in cloud. The edited waypoints need a new name for sharing.
...
Before using the waypoints library module, developer need to setup the workspace ID in H5 through JSBridge, configure API module and then load the DJI Pilot 2 mission module. Also, developers can consider adding the loading interface of mission module in log-in phase.
...
After loading the Mission waypoints library module, DJI Pilot 2 will pull the list of waypoints files from the server via HTTP and display them in the DJI Pilot 2 waypoints library.
...
When user clicks to download waypoints in the DJI Pilot 2 waypoints list, the DJI Pilot 2 will initiate a request to get the waypoints download address.
...
After the waypoints file is uploaded to the object storage service, DJI Pilot 2 will report the upload result to the server side, which will perform the corresponding business operations on the result.
...
When users need to pay special attention to waypoint files, they can click on Favorites. At the same time, the interface will be called to mark the waypoints files that need to be favored.
